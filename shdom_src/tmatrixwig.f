      SUBROUTINE TMATRIXWIG (wavelength, mindex, radius,
     .                       cylflag, axratio, maxrank,
     .                       extcross, scatcross, ssalbedo, asymmetry,
     .                       nrank, wigcoef)

C      Computes the light scattering properties of a single randomly
C      oriented spheroid or cylinder using the T-matrix code of M. Mishchenko.
C      Spheroids are calculated with cylflag=.false. and cylinders with 
C      cylflag=.true.  The axis ratio (axratio) is > 1 for an oblate 
C      spheroid and < 1 for a prolate spheroid.   For cylinders axratio
C      is the ratio of the diameter to the length.   The radius is 
C      the equal volume radius. The imaginary part of the complex index 
C      of refraction specified in mindex can have either sign, as it is 
C      set to positive when used.  
C      The Wigner d-function coefficients of the six unique elements of 
C      the scattering phase matrix are returned in wigcoef(6,0:maxleg) in 
C      order alpha1, alpha2, alpha3, alpha4, beta1, beta2.
C      The inputs and outputs to this subroutine are in REAL*4, but
C      the T-matrix calculations are done in REAL*8.
C
C      This interface was written by Frank Evans  (May 2011)
C        Modified for Wigner d-functions  (January 2014)
C
C        Compile: pgf77 -c tmatrixdwig.f lpd.f  (tmd.par.f include file used)
C
C      The main code was modified to put in the subroutine interface at
C      the beginning, set parameters for a single particle, and calculate
C      the Legendre coefficients of the six unique elements of the phase
C      matrix (first make Gaussian quadrature angles, call MATR to get the
C      scattering matrix at these angles, and then do the integration over
C      Wigner functions).  The MATR subroutine was modified to output the 
C      scattering matrix at specified angles.  
      IMPLICIT REAL*8 (A-H,O-Z)
      INTEGER  maxrank, nrank
      LOGICAL  cylflag 
      REAL     wavelength, radius, axratio
      COMPLEX*8  mindex
      REAL     extcross, scatcross, ssalbedo, asymmetry
      REAL     wigcoef(6,0:maxrank)

C      Comments below are from the original tmd.lp.f code.
...
C   ANY DAMAGES THAT MAY RESULT FROM THE USE OF THE PROGRAM. 
     ...
     *        DDR(NPNG2),DRR(NPNG2),DRI(NPNG2),ANN(NPN1,NPN1)
     ...
     &     IT21(NPN6,NPN4,NPN4),IT22(NPN6,NPN4,NPN4)
      PARAMETER (maxnang=500)
      REAL*8 muang(maxnang), wts(maxnang), scatmat(6,maxnang)
      REAL*8 coef(6,0:NPL), backscatvec(6)
      REAL*8 D00(0:maxnang), D20(0:maxnang)
      REAL*8 D22P(0:maxnang), D22M(0:maxnang)
      REAL*8 A1, A2, A3, A4, B1, B2, F
...
C  INPUT DATA ********************************************************
      LAM = wavelength
      MRR = REAL(mindex)
      MRI = ABS(AIMAG(mindex))
C       NP=-1 for spheroids, NP=-2 for cylinders
      IF (cylflag) THEN
        NP = -2
      ELSE
        NP = -1
     ...
     &          'IN SIZE AVERAGING =',I4)
         ...
         DO 500 INK=1,NK
            ...
            if (INM1.GE.NPN1) then
              PRINT 7333, NPN1
              print '(A,F9.4,A,F5.2,A,2F8.4)',
     &             'radius=',radius, '  axratio=',axratio, '  m=',mindex
              return 
           endif
...
c          Calculate the backscattering cross sectioun
c         CALL MATR (ALPH1,ALPH2,ALPH3,ALPH4,BET1,BET2,LMAX, 
c     .              1, -1.0D0, backscatvec)
c         backscat = scatcross*backscatvec(1)

c         Code to calculate Wigner d coefficients of the phase matrix (Evans)
         Nrank = MIN(L1MAX,maxrank)
         Nquad = 2*L1MAX
         IF (Nquad .GT. maxnang) 
     .      STOP 'tmatrixwig: Number of angles exceeded'
c          Make the Gaussian quadrature angles and get scattering matrix for them
         CALL GAUSS (Nquad,0,0,muang,wts)
         CALL MATR (ALPH1,ALPH2,ALPH3,ALPH4,BET1,BET2,LMAX, 
     .              Nquad,muang, scatmat)
         DO I = 1, Nquad
           DO J = 1, 6
             scatmat(J,I) = scatmat(J,I)*wts(I)
           ENDDO
         ENDDO
         DO L = 0, Nrank
           DO J = 1, 6
...
C             l=0 to NRANK for muang(I)
           CALL WIGNERFCT (muang(I), Nrank, 0, 0, D00)
           CALL WIGNERFCT (muang(I), Nrank, 2, 2, D22P)
           CALL WIGNERFCT (-muang(I), Nrank, 2, 2, D22M)
           CALL WIGNERFCT (muang(I), Nrank, 2, 0, D20)  
c            scatmat elements in old Evans order (JQSRT 1991)
           A1 = scatmat(1,I)  
           B1 = scatmat(2,I)
           A3 = scatmat(3,I)
           B2 = scatmat(4,I)
           A2 = scatmat(5,I)
           A4 = scatmat(6,I)
C           Calculate the Wigner d-function series expansion coefficients
           DO L = 0, Nrank
             F = (-1.0D0)**L
             coef(1,L) = coef(1,L) +(L+0.5D0)*A1*D00(L)
             coef(2,L) = coef(2,L) +(L+0.5D0)*(A2+A3)*D22P(L)
             coef(3,L) = coef(3,L) +(L+0.5D0)*(A2-A3)*F*D22M(L)
             coef(4,L) = coef(4,L) +(L+0.5D0)*A4*D00(L)
             coef(5,L) = coef(5,L) -(L+0.5D0)*B1*D20(L)
c              Flip the sign of the B2 (F34) coefficient to match my convention as in make_mie_table
             coef(6,L) = coef(6,L) +(L+0.5D0)*B2*D20(L)
           ENDDO
         ENDDO
         DO L = 0, Nrank
           ...
           coef(3,L) = A3
         ENDDO
c          Normalize the coefficients (return Wigner d coefs times scattering cross section)
         DO L = 0, Nrank
           DO J = 1, 6  
             wigcoef(J,L) = SNGL( scatcross*coef(J,L)/coef(1,0) )
           ENDDO
         ENDDO  
...
C     Matrix inversion from LAPACK 
 ...
 1100 FORMAT ('WARNING: matrix inversion error info=', i6)
      DO I=1,NNMAX
         DO J=1,NNMAX
            TR=0D0
            TI=0D0
...
C    LMAX   - NUMBER OF COEFFICIENTS MINUS 1
C    Nang   - NUMBER OF SCATTERING ANGLES
C    muang  - array of cosine of the angles
C    scatmat - six scattering matrix elements (F11,F12,F33,F34,F22,F44)
C               for each angle.
 
      SUBROUTINE MATR(A1,A2,A3,A4,B1,B2,LMAX, Nang,muang, scatmat)
      IMPLICIT REAL*8 (A-H,O-Z)
      INTEGER Nang
      REAL*8  muang(Nang), scatmat(6,Nang)
      INCLUDE 'tmd.par.f'
